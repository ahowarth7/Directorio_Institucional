...
Ext.extend(Ext.grid.XPrinter , Ext.util.Observable,{
	...
	prepare: function(){
		...
		if  (this.useCustom.customTPL){ 
			if (this.pdfEnable){ 
				this.PrinterWindow = window.open( this.pathPrinter + "/base_print_002.php?xlsgen=1&pdfgen=1&paper="+ this.paperOrientation  ,"PrinterWindow","width=" + this.wWidth + ",height=" + this.wHeight + ",resizable=1,scrollbars=1");			
			} else { 
				this.PrinterWindow = window.open( this.pathPrinter + "/base_print_002.php?xlsgen=1&paper=" + this.paperOrientation ,"PrinterWindow","width=" + this.wWidth + ",height=" + this.wHeight + ",resizable=1,scrollbars=1");				
			}
			this.PrinterWindow.focus();
		} else {  // normal printing on grid base or store base 
			if (this.pdfEnable){ 
				this.PrinterWindow = window.open( this.pathPrinter + "/base_print_001.php?xlsgen=1&pdfgen=1&paper="+ this.paperOrientation  ,"PrinterWindow","width=" + this.wWidth + ",height=" + this.wHeight + ",resizable=1,scrollbars=1");			
			} else { 
				this.PrinterWindow = window.open( this.pathPrinter + "/base_print_001.php?xlsgen=1&paper=" + this.paperOrientation ,"PrinterWindow","width=" + this.wWidth + ",height=" + this.wHeight + ",resizable=1,scrollbars=1");
			}
			this.PrinterWindow.focus(); 
		}
	...
	getDatareport:function(){ 
		...
		var bodytpl=new Ext.XTemplate(	
			...
			'<img src="images/page_excel.png" width="16" height="16" onclick="genXLS();" /> <a href="javascript:window.genXLS();"> Excel</a> | ',
            ...
            */			
			...
			'<tr><td width="15%" rowspan="3"> <img src="' + this.logoURL + '" width="90" height="70"/></td>',
			'<td width="85%" class="date"><div align="left"></div></td></tr>',
			'<tr><td><div class="title">{title}</div></td></tr>',
			'{subtitle:this.chksubtitle}',
			'<tr><td class="date"> <div style="font-size:7px;text-align:left;">{date}<div> </td></tr></table>',
			'<hr noshade="noshade" width="100%" align="left"/>',
			'<div id="data-table">{valuebody}</div>',
			'<table width="100%" border="0" cellspacing="3" cellpadding="0"><tr><td colspan="2" class="subtitle">',
			'<hr noshade="noshade" width="100%" align="left"/></td></tr><tr>',
		...
		if (this.useCustom.custom==false){  //use store grid and grid settings
					...
					columnstext = '<table width="100%" border="0" cellspacing="1" cellpadding="0" ><tr>'; 
