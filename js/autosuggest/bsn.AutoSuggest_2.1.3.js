...
_b.AutoSuggest.prototype.getSuggestions = function (val)
{
	...
	//
	var l = this.aSug.length; val = val.removeAccents();
	if (this.nInpC > ol && l && l<this.oP.maxentries && this.oP.cache)
	{
		var arr = [];
		for (var i=0;i<l;i++)
		{   
		    str1 = val.removeAccents();
			str2 = this.aSug[i].value.substr(0,val.length).removeAccents();
				
			if (str1.toLowerCase() == str2.toLowerCase() || this.aSug[i].info == 'as_header' )
...
_b.AutoSuggest.prototype.doAjaxRequest = function (input)
{
	...
	//valores de los campos que debo de obtener antes de hacer la peticion
	var obj = this.oP.getFields;
	for(key in obj) {
		var val  = $(obj[key]).val();
		if(!val) val='';
		
		var str = "this.oP.params."+key+"='"+val+"'";
		try {eval(str);}catch(e){alert(e.message)}
				
    }
		
	var params = this.oP.params;
...
_b.AutoSuggest.prototype.createList = function(arr)
{
	...
	if (arr.length == 0 && this.oP.shownoresults)
	{
		...
		li.onclick = function(){ pointer.clearSuggestions(); pointer.oP.handleNoResult(); return false; } 
		ul.appendChild( li );
...
_b.AutoSuggest.prototype.resetTimeout = function()
{
	clearTimeout(this.toID);
	var pointer = this;
	this.toID = setTimeout(function () { pointer.clearSuggestions("fade") }, 2000);
};
