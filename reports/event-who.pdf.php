...
class MyPDF extends fpdf_table{
	...
	*----------------------------------------------------------------------------------------------------------*/
	...
	public function drawPDF (){
		...
		$filters = array(
		    'recibio'   => $post['recibio'],
			'confirmo'  => $post['confirmo'],
			'cquien'    => $post['quien'],
			'groupby'   => 'even_grupo',
			'limit'     => 1000000
		);
		//empezamos por los grupos, al menos uno de los grupos debe tener una persona
		$rgrupos = $rinvitados = $event->getSeguimiento($post['evento'], $filters, 'array');
		...
		if(is_array($rgrupos)){
		   foreach($rgrupos as $key => $rgrupo){
			   ...
			   $this->Ln(2);
			   $idGrupo = $rgrupo['even_grupo'];
			   $this->SetLineWidth(.2);
			   $this->SetFillColor(220,230,242); //213,217,214 
			   $this->SetFont('Times','B',10);
			   $this->Cell(0,4, $rgrupo['even_ngrupo'] , $b='B', $ln=1, $a= 'L', $f = true, $link = '');
			   ...
			   $filters = array(
				  ...
				  'confirmo'  => $post['confirmo'],
				  'cquien'    => $post['quien'],
				  'groupby'   => 'even_dependencia',
				  'limit'     => 1000000
			   );
			   //dependencias del grupo
			   $rdependencias = $event->getSeguimiento($post['evento'], $filters ,'array');
			   if(is_array($rdependencias)){
				  foreach($rdependencias as $kdep => $rdependencia){
				       if($rdependencia['descrip']=='TODOS') continue;
					   
					   $idDependencia = $rdependencia['even_dependencia'];
					   $this->Ln(2);
					   ...
					   $rinvitados = $event->getSeguimiento($post['evento'], $filters,'array');
					   if(is_array($rinvitados)){
						  $index = 0;
						  
						  $this->SetFont('Times','B',9);
					      $this->Cell(5,3,'');
			              $this->Cell(0,3, $rdependencia['even_ndependencia']. "  (".count($rinvitados).")" , $b='B', $ln=1, $a= 'L', $f = false, $link = '');
						  ...
						  foreach($rinvitados as $kinvi => $rinvitado){
							  ...
							  $npersona = $rinvitado['even_npersona'];
							  
							  if($post['cquien']=='S') $npersona= $npersona;
							  
							  $this->Row(array('',++$index,$npersona),array(0,0,0,0,0,0));
							  $this->SetWidths(array(3,6,240),array('L','L','L'),4,0);
